...
impl Signer<Ed25519> for Ed25519DidWebJwk {
    ...
    async fn generate_jwt(&self, vc: &Credential) -> Result<String, SignerError> {
        Ok(vc
            ...
            .map_err(|e| SignerError::InvalidSignerOpts {
                signer_type: self.signer_type.name(),
                reason: format!("Failed to generate JWT: {}", e.to_string()),
            })?)
    }
    ...
    async fn proof(&self, vc: &Credential) -> Result<Option<OneOrMany<Proof>>, SignerError> {
        ...
        Ok(Some(OneOrMany::One(
            vc.generate_proof(&self.key, &lpdo, &DIDWeb, &mut context_loader)
                .await
                .map_err(|e| SignerError::InvalidSignerOpts {
                    signer_type: self.signer_type.name(),
                    reason: format!("Failed to generate LDP proof: {}", e.to_string()),
                })?,
        )))
    }
